...
window.addEventListener('DOMContentLoaded', function () {
  ...
  window.addEventListener('load', function () {
    ...
    $(document).mouseup( function(e){
      var div = $("[data-burger-list]");
      if (!div.is(e.target) && div.has(e.target).length === 0) {
        $('html').removeClass('of-hidden');
        $('[data-burger]').removeClass('is-active');
        $('[data-burger-list]').removeClass('is-active');
      }
    });

    var reviewSlider = document.querySelectorAll('.reviews__slider');
    if (reviewSlider) {
      reviewSlider.forEach(function (slider) {
    ...
    if (window.innerWidth > 1030) {
      var partners = document.querySelectorAll('.partners__slider');
      if (partners) {
    ...
    $('[name="payment-type"]').on('change', function () {
      if ($(this).val() == 'рассрочка') {
        var $this = $(this),
          $parent = $this.parents('.cost__form-type__wrap'),
          $rassr = $parent.find('[name="rassr-type"]');
        if ($rassr) {
          $rassr.eq(0).attr('checked', 'true');
        }
        $parent.addClass('is-active');
      } else {
        // $('.gift__input').removeAttr('required');
        $('[name="rassr-type"]').eq(0).removeAttr('checked');
        $('[name="payment-type"][data-options]').parents('.cost__form-type__wrap').removeClass('is-active');
      }
    });
    $('[name="gift"]').on('change', function () {
      var $this = $(this),
        $parent = $this.parents('.cost__form-type__wrap'),
        $fields = $parent.find('.gift__input');
